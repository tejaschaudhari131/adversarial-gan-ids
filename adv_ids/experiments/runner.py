...
from adv_ids.evaluation.plots import save_attack_comparison, save_transfer_heatmap
    ...
    if name == "gan":
        ...
        if key not in gan_cache:
            ...
            gan.fit(
                ...
                artifacts_dir=artifacts / f"gan_{model.name}",
            )
            gan_cache[key] = gan
...
def run_experiment_suite(config: dict[str, Any] | str | Path, root: str | Path = ".") -> dict[str, Any]:
    ...
    for raw_ds in config.get("datasets", ["cicids2017"]):
        ...
        ds_art = ensure_dir(artifacts_root / ds_name)
        data = prepare_dataset(
            path,
        ...
        for model_name in config.get("models", ["mlp"]):
            ...
            clean = evaluate_clean(model, data["X_test"], data["y_test"], results_dir=out_root / ds_name / model.name)
            write_json(out_root / ds_name / model.name / "clean.json", {k: v for k, v in clean.items() if k != "classification_report"})
            ...
            for raw_atk in config.get("attacks", ["fgsm", "pgd"]):
                ...
                metrics = evaluate_attack_on_model(
                    ...
                    results_dir=out_root / ds_name / model.name, tag=atk["name"],
                )
                row = {
                    "dataset": ds_name,
                    "model": model.name,
                    ...
                    "label": f"{ds_name}/{model.name}/{atk['name']}",
                }
        ...
        if defense_cfg.get("name") in {"adv_train", "adversarial_training", "pgd_train"}:
            ...
            for raw_atk in config.get("attacks", ["fgsm", "pgd"]):
                ...
                metrics = evaluate_attack_on_model(
                    ...
                    results_dir=out_root / ds_name / defended.name, tag=atk["name"],
                )
                summary_rows.append({
                    "dataset": ds_name,
                    "model": defended.name,
                    ...
                    "attack_success_rate": metrics["attack_success_rate"],
                    "mean_l2_perturbation": metrics["mean_l2_perturbation"],
                    "label": f"{ds_name}/{defended.name}/{atk['name']}",
                })
            ...
            if surrogate is None or not surrogate.differentiable:
                ...
                for tname in targets:
                    ...
                    metrics = evaluate_attack_on_model(
                        ...
                        results_dir=out_root / ds_name / "transfer", tag=f"{surrogate.name}_to_{target.name}_{attack_name}",
                    )
                    row = {
